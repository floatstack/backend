...
function addGaussianNoise(value: number, stdDev: number): number {
...
function createBoundarySample(type: 'low-to-balanced' | 'balanced-to-rich'): Sample {
  ...
  const features = [
    e_float_percentage,
    rand(100_000, 400_000) / 100_000,
    rand(100_000, 400_000) / 100_000,
    rand(20_000, 60_000) / 100_000,
    rand(5, 30) / 100,
    hour_sin,
    hour_cos,
    Math.random() > 0.5 ? 1 : 0,
...
function generateUrbanAgent(): Sample[] {
  ...
  for (let i = 0; i < SAMPLES_PER_TYPE; i++) {
    ...
    const baseVelocity = isPeak ? rand(300_000, 600_000) : rand(150_000, 400_000);
    ...
    const cash_balance = addGaussianNoise(rand(200_000, 500_000), 30_000);
    const refill_amount = addGaussianNoise(rand(20_000, 80_000), 5_000);
    const refill_hrs = addGaussianNoise(rand(2, 12), 1);

    const features = [
      Math.max(0.1, e_float_percentage), // Ensure positive
      velocity / 100_000,
      Math.max(0, cash_balance) / 100_000,
      Math.max(0, refill_amount) / 100_000,
    ...
    const label: 0 | 1 | 2 =
      features[0] < 0.6 ? 0 : features[0] > 1.3 ? 2 : 1;
...
function generateRuralAgent(): Sample[] {
  ...
  for (let i = 0; i < SAMPLES_PER_TYPE; i++) {
    ...
    const velocity = addGaussianNoise(rand(50_000, 150_000), 15_000);
    const cash_balance = addGaussianNoise(rand(30_000, 120_000), 10_000);
    const refill_amount = addGaussianNoise(rand(10_000, 50_000), 5_000);
    const refill_hrs = addGaussianNoise(rand(12, 48), 3);

    const features = [
      Math.max(0.1, e_float_percentage),
      Math.max(0, velocity) / 100_000,
      Math.max(0, cash_balance) / 100_000,
      Math.max(0, refill_amount) / 100_000,
...
function generateIdleAgent(): Sample[] {
  ...
  for (let i = 0; i < SAMPLES_PER_TYPE; i++) {
    ...
    const velocity = addGaussianNoise(rand(10_000, 80_000), 8_000);
    const cash_balance = addGaussianNoise(rand(200_000, 450_000), 40_000);
    const refill_amount = addGaussianNoise(rand(15_000, 60_000), 8_000);
    ...
    const label: 0 | 1 | 2 = features[0] > 1.3 ? 2 : 1;
    samples.push({ features, label });
  }
  return samples;
}
